...
export default function Header({ onOpenAdd, onSetTab, activeTab, user, onLogin, onSignup, onLogout, isAdmin = false }) {
  ...
  return (
    <React.Fragment>
      <div className="flex justify-between items-center px-10 py-5 bg-white shadow-sm">
        ...
        <div className="flex items-center gap-3">
          {user ? (
            ...
            <>
              <button className="px-4 py-2 bg-transparent border border-teal-600 text-teal-600 rounded-lg" type="button" data-auth-trigger="login" onClick={() => setShowLogin(true)}>Login</button>
              <button className="px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg" type="button" onClick={() => setShowSignup(true)}>Sign Up</button>
            </>
          )}
        </div>
      </div>
      ...
      {showSignup && (
        <SignupModal onClose={() => setShowSignup(false)} onSignup={onSignup} />
      )}
      {showForgot && (
        <ForgotPasswordModal
